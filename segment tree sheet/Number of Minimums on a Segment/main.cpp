...
pair<long long,long long> segmenttreeans(int l,int r,int s,int e)
{
    // cout << l << " "<< r << " " << s <<  "  " << e << endl;
    if(l==r&&s<=l&&e>=r)
    {//cout << tree[N+l-1]<<endl;
        return {sum[N+l-1],tree[N+l-1]};
    }
    if(s<=l&&e>=r)
    {
        int div=r-l+1;
    ...
    {//cout << 0<< endl;
        ...
        int mid=(l+r)>>1;
pair<long long,long long> m1=segmenttreeans(l,mid,s,e);
pair<long long,long long> m2= segmenttreeans(mid+1,r,s,e);
     if(m1.second==m2.second)
     {
         return {m1.first+m2.first,m1.second};
     }
...
void updatenode(int l,int r,int pos,long long val)
{
    if(l==r&&l==pos)
    {
        ...
        while(node>0)
        {
            ...
            else if(tree[node]==tree[node<<1])
            {
                sum[node]=sum[node<<1];
            }
            else
                sum[node]=sum[(node<<1)|1];
...
int main() {
    ...
    while(q--)
    {
        ...
        {
            auto m=segmenttreeans(1,N,y+1,z);
